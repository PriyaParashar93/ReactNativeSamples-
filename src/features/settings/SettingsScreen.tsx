        ...
        {options.map((option, index) => (
          ...
          >
            <View style={styles.rowLeft}>
              <View style={[styles.iconWrap, option.id === "logout" && styles.iconWrapRed]}>
                <Icon name={option.icon} size={22} color={option.color} />
              </View>
              <Text style={[styles.label, option.id === "logout" && styles.labelRed]}>
                {option.label}
              </Text>
            </View>
            {option.id !== "logout" && (
...
const styles = StyleSheet.create({
  ...
  titleRow: { paddingHorizontal: 20, paddingTop: 24, paddingBottom: 16 },
  ...
  iconWrapRed: { backgroundColor: "#fdecea" },
  label: { fontSize: 16, color: "#222", fontWeight: "500" },
  labelRed: { color: "#e53935" },
});
